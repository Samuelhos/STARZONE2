...
function loadWebsiteData(data){
  ...
  if(nintendoPrice) nintendoPrice.innerHTML = `Rp ${data.nintendoPrice || DEFAULT_DATA.nintendoPrice} / hari`;

  const playboxPrice = document.getElementById("playboxPriceText");
  if(playboxPrice) playboxPrice.innerHTML = `Rp ${data.playboxPrice || DEFAULT_DATA.playboxPrice} / hari`;
...
// Opening saja: ada Skip, hilang otomatis.
// =========================
(function initStarZoneOpening(){
  let removeTimer=null, hideTimer=null;
  function hideOpening(){
    ...
    removeTimer=window.setTimeout(()=>{if(loader&&loader.parentNode){loader.parentNode.removeChild(loader)}},800);
  }
  function createOpeningEffects(){
    ...
    if(!loader||!stars||!streaks) return;
    document.body.classList.add("sz-intro-lock");
    if(skipBtn){skipBtn.addEventListener("click",hideOpening)}
    stars.innerHTML="";
    for(let i=0;i<70;i++){
  ...
  if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",createOpeningEffects)}else{createOpeningEffects()}
})();
